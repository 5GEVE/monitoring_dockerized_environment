...
if [[ $TOPIC == *"metric."* ]]; then
	echo "input { kafka { bootstrap_servers => '${KAFKA_IP_PORT}' client_id => 'logstash' topics => '$TOPIC' } } filter { ruby { code => \"event.set('message_size', event.get('message').bytesize)\" } json { source => \"message\" } if \"_jsonparsefailure\" in [tags] { drop { } } else { split { field => \"records\" } if \"_split_type_failure\" in [tags] { drop { } } else { mutate { rename => { \"[records][value][metric_value]\" => \"metric_value\" \"[records][value][timestamp]\" => \"timestamp\" \"[records][value][unit]\" => \"unit\" \"[records][value][device_id]\" => \"device_id\" \"[records][value][context]\" => \"context\" } remove_field => [\"message\"] convert => { \"metric_value\" => \"float\" } } date { match => [\"timestamp\", \"UNIX\"] target => \"device_timestamp\" remove_field => \"timestamp\" } } } } output { elasticsearch { hosts => \"${ELASTICSEARCH_IP_PORT}\" index => \"$TOPIC_LOWERCASE\" user => \"$USER\" password => \"$PASSWORD\" } }" | tee -a /etc/logstash/conf.d/${TOPIC}.conf > /dev/null;
elif [[ $TOPIC == *"kpi."* ]]; then
